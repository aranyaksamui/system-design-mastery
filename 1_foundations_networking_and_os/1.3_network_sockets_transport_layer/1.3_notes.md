...
> **O4. OSI Reference Model along with the protocols and technologies at each layer.** 

**Answer:**

| Layer | Name | Protocols / Technologies | Core Responsibilities |
| ------ | ------ | ------ | ------ |
| **7** | **Application** | HTTP, HTTPS, DNS, FTP, SFTP, SMTP, IMAP, POP3, SSH, Telnet. | * Provides the network interface and standardized services for user applications.<br>* Defines strict rules for how requests must be formatted so the receiving server understands them.<br>* Wraps data into a highly specific text format with headers to create the payload. |
| **6** | **Presentation** | SSL/TLS, H.264, Gzip, Brotli, JSON, Protocol Buffers. | * Acts as the data's syntax interpreter by translating data into standardized network formats.<br>* Shrinks the payload by compressing data to save bandwidth.<br>* Scrambles data into ciphertext using encryption (like TLS) to keep the contents private. |
| **5** | **Session** | OS Socket APIs. | * Acts as the communication coordinator by establishing, managing, and gracefully terminating logical connections (sessions).<br>* Manages dialog control (half-duplex or full-duplex) between devices.<br>* Injects synchronization points (checkpoints) into data streams so failed transfers can resume without starting over. |
| **4** | **Transport** | TCP, UDP. | * Prepares software data for the hardware layers by providing process-to-process delivery using Ports.<br>* Chops large, continuous streams of data into smaller chunks called segments.<br>* Dictates reliability through TCP (using 3-way handshakes, sequencing, and retransmitting lost data) or raw speed through UDP (fire-and-forget). |
| **3** | **Network** | IPv4, IPv6, OSPF, BGP. | * Encapsulates segments into packets and assigns logical addressing (Source and Destination IP addresses).<br>* Uses routing tables (built via OSPF for internal networks and BGP across the internet) to make hop-by-hop forwarding decisions.<br>* Enforces a Time-To-Live (TTL) limit to prevent packets from looping infinitely. |
| **2** | **Data Link** | Ethernet, MAC Addressing. | * Manages node-to-node data transfer across a single physical link.<br>* Encapsulates IP packets into frames and assigns physical MAC addresses burned into the Network Interface Card (NIC).<br>* Adds a checksum (error detection) to discard frames that suffered bit corruption during the physical hop. |
| **1** | **Physical** | Ethernet cables, fiber optic glass, Wi-Fi radio waves, transceivers. | * Deals exclusively with the raw transmission medium and physical hardware components.<br>* Converts logical binary bits (0s and 1s) from Layer 2 frames into physical electrical voltage pulses, flashes of light, or radio signals.<br>* Contains no concept of addressing; it simply transmits the raw signals. |
